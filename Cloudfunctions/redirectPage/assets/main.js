...
(function() {
  ...
  const scrollToElement = function(e, style, duration) {
    let y; let cy; let dy;
    let start; let easing; let offset; let f;
  ...
  var scrollEvents = {
    ...
    /**
			 ...
			 */
    handler: function() {
      let	height; let top; let bottom; let scrollPad;
      ...
      scrollEvents.items.forEach(function(item) {
        let bcr; let elementTop; let elementBottom; let state; let a; let b;
  ...
  const onvisible = {
    ...
    add: function(selector, settings) {
      ...
      const offset = parseInt("offset" in settings ? settings.offset : 0);
      const mode = parseInt("mode" in settings ? settings.mode : 3);
      const replay = "replay" in settings ? settings.replay : false;
      const stagger = "stagger" in settings ? (parseInt(settings.stagger) / 1000) : false;
      const state = "state" in settings ? settings.state : null;
      const effect = this.effects[style];

      // Step through selected elements.
      $$(selector).forEach(function(e) {
        const	children = (stagger !== false) ? e.querySelectorAll(":scope > li, :scope ul > li") : null;
        const enter = function(staggerDelay=0) {
          let	_this = this;
          let transitionOrig;
        ...
        let targetElement; let triggerElement;
  ...
  onvisible.add("#container03", {style: "zoom-in", speed: 500, intensity: 5, delay: 0, replay: false});
})();
